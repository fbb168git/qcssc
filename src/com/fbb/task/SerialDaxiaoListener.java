...
public class SerialDaxiaoListener extends TimerTask{
	static final int TipsCount = 4;
...
//		ArrayList<Lottery> lotterys = lotteryDao.getByStartDate("2017-01-01", "2017-1-01");
...
//			","
	...
	private void notifyListenerQian2() {
		if(cacheQian2.size() >= TipsCount) {
			...
			SMSUtil.sendSms(smsMsg, lottery.getLottery_nums(), lottery.getLottery_time(),"15838948865");
			if(cacheQian2.size() >= TipsCount + 1) {
				SMSUtil.sendSms(smsMsg, lottery.getLottery_nums(), lottery.getLottery_time(),"18701501627");
			}
		}
	}
